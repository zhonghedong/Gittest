...
class Login(object):
    def __init__(self, server):
        ...
        self.__loginsuc = "0"
        self.__passwderror = "1"
        self.__usernameerror = "2"
        self.__loginrepeat = "3"
        self.__friendsfile = "friends.txt"
        self.__friendsfilepath = os.path.dirname(os.path.dirname(__file__)) + u"/friends/" + self.__friendsfile
        ...
        for user in registeredusers:
            if body[u"content"][u"name"] == user[u"name"]:
                if body[u"content"][u"passwd"] == user[u"passwd"]:
                    ...
                    self.__server.addloginuser(currentuser)
                    self.dealLoginxx(serversock, self.__loginsuc, body)
                    return
                else:
                    print u"密码错误"  # 用户名正确，密码错误
                    self.dealLoginxx(serversock, self.__passwderror, body)
                    return
        print u"用户名错误"
        self.dealLoginxx(serversock, self.__usernameerror, body)
    ...
    def dealLogin(self, serversock, currentuser, body):
        ...
        if not self.isLogined(body[u"content"][u"name"]):
            ...
            self.dealLoginxx(serversock, self.__loginrepeat, body)
        f.close()
